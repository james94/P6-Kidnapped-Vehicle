...
/**
 * Computes Multi-variate Gaussian Distribution
 * 
 * Referenced Particle Weights Solution from Udacity SDCE-ND T2L5
 */
inline double multiv_prob(double sig_x, double sig_y, double x_tobs, double y_tobs,
  ...
  // calculate exponent
  double exponent;
  exponent = ( pow(x_tobs - mu_x, 2) / (2 * pow(sig_x, 2)) ) +
             ( pow(y_tobs - mu_y, 2) / (2 * pow(sig_y, 2)) );
